...
def test_prediction_simulation_observables() -> None:
    ...
    engine.encode("Module gamma firmware.", pin=True)
...
def test_attention_and_accessibility_influence_prediction() -> None:
    engine = CognitiveEngine(agent_id="psobs")
    a = engine.encode("Priority toolkit alpha.", pin=True)["concept_id"]
    b = engine.encode("Neglected trivia zeta.", pin=True)["concept_id"]
    ...
    if labels:
        hit = any(
            "toolkit" in lab or "alpha" in lab or "priority" in lab for lab in labels
        )
