...
def test_association_emergence_and_strengthening() -> None:
    engine = CognitiveEngine(agent_id="asc")
    for _ in range(4):
        engine.encode("Fly tying and craftsmanship go together.", pin=True)
...
def test_asymmetric_is_a_traffic() -> None:
    ...
    ties = [
        a
        for a in engine.store.associations.values()
        if a.relation == RelationKind.IS_A_TRAFFIC
    ]
    assert ties
    edge = ties[0]
...
def test_weakening_and_dormancy() -> None:
    engine = CognitiveEngine(agent_id="asc")
    engine.encode("Obscure widget pairs with gizmos.", pin=True)
...
def test_neighborhood_and_clusters() -> None:
    ...
    engine.encode("Tool use belongs with woodworking.", pin=True)
...
def test_identity_and_goal_influence_stamps() -> None:
    ...
    stamped = [
        a
        for a in engine.store.associations.values()
        if a.goal_influenced or a.identity_influenced
    ]
    assert stamped
